  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('pricing_plans').select('id, name, price, billing_period, is_recommended, price_per_credit, features').order('price', {
        ascending: true
      });
  ...
  return <Container className="max-w-screen-xl px-2 sm:px-4 md:px-6">
      ...
      <div className="w-full overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {plansLoading || userPlanLoading ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[1, 2].map(i => <div key={i} className="border rounded-xl p-6 space-y-4">
              ...
              <TabsContent value="subscription" className="mt-0">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {subscriptionPlans?.map(plan => <PlanCard key={plan.id} plan={plan as PlanData} isActive={isPlanActive(plan.id)} onSelect={setSelectedPlanId} features={getPlanFeatures(plan.name)} planType="subscription" />)}
                </div>
                
                <SubscriptionManager selectedPlanId={selectedPlanId} isActivePlan={selectedPlanId ? isPlanActive(selectedPlanId) : false} isProcessing={isProcessing} setIsProcessing={setIsProcessing} />
              </TabsContent>
              
              <TabsContent value="credits" className="mt-0">
                {creditPlans?.map(plan => <div key={plan.id} className="mb-6">
                    ...
                    <CreditPackageManager pricePerCredit={plan.price_per_credit || 0.001} userPlan={userPlan} />
                  </div>)}
              </TabsContent>
            </>}
        </Tabs>
